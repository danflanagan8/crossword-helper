...
function putWordsIntoDictionary( wordArray ){
	...
	while( wordArray.length > 0 ){
	   ...
	   dictionary.add( reverseWord(wordArray[rand_index]) );
...
function printMatchArray( matchArray ){
	...
	matchArray.forEach( function(entry){
		var text = document.createTextNode(reverseWord(entry) + "\n");
...
function addWord( word ){
    ...
    if( word.indexOf("-") == -1 && word.indexOf("*") == -1 && word.indexOf("#") == -1 && word.indexOf("^") == -1){
       ...
       dictionary.add(reverseWord(word.toLowerCase()));
...
function removeWord( word ){
    ...
    if( word.indexOf("-") == -1 && word.indexOf("*") == -1 && word.indexOf("#") == -1 && word.indexOf("^") == -1){
       ...
       dictionary.remove(reverseWord(word.toLowerCase()));
...
function putDansWordsInDictionary(){
   ...
   words.forEach( function( entry ){
	   ...
	   dictionary.add(reverseWord(entry));
...
function findAndShowMatches(){
    ...
    var to_match = new wildCardWord( reverseWord(document.getElementById('wildWord').value) );
...
function findAndShowGrid(){
   ...
   //rotate the words 180 degrees
   words = rotateGrid(words);
   //remove blank lines
...
function rotateGrid(words){
  ...
  for( var row = 0 ; row < words.length ; row++){
    rotated[row] = "";
    for( var col = 0; col < words[0].length ; col++){
      rotated[row] += words[words.length - 1 - row][words[0].length - 1 - col];
    }
  }
  console.log(rotated);
  return rotated;
